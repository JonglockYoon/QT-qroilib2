	...
	for( it;it!=en;it++)
	{
		...
		else
		{
            if( !maskImage.empty() )
			{
                ...
                pMask = (char *)(maskImage.data + actualPoint.x - 1 + (actualPoint.y - 1) * maskImage.step);
				...
				if(!find)
				{
                    pMask = (char *)(maskImage.data + actualPoint.x - 1 + (actualPoint.y ) * maskImage.step);
...
double CBlob::Mean( cv::Mat image )
{
    ...
    //mask = cv::Mat( cv::Size(m_boundingBox.width, m_boundingBox.height), CV_8UC1);
    //cvSetZero(mask);
    Mat mask_mat = cv::Mat::zeros(cv::Size(m_boundingBox.width, m_boundingBox.height), CV_8UC1);
	...
	if(isJoined){
		list<CBlob *>::iterator it,en = joinedBlobs.end();
		for(it = joinedBlobs.begin();it!=en;it++){
// 			cvDrawContours( mask, (*it)->m_externalContour.GetContourPoints(), CV_RGB(255,255,255), CV_RGB(255,255,255),0, cv::FILLED, 8,
// 				offset );
// 			vector<t_PointList> conts;
// 			conts.push_back((*it)->m_externalContour.GetContourPoints());
            drawContours(mask_mat,(*it)->m_externalContour.GetContours(),-1,CV_RGB(255,255,255),cv::FILLED,8,noArray(),2147483647,offset);
			t_CBlobContourList::iterator itint = (*it)->m_internalContours.begin();
			while(itint != (*it)->m_internalContours.end() )
			{
// 				cvDrawContours( mask, (*itint).GetContourPoints(), CV_RGB(0,0,0), CV_RGB(0,0,0),0, cv::FILLED, 8,
// 					offset );
                drawContours(mask_mat,(*itint)->GetContours(),-1,CV_RGB(0,0,0),cv::FILLED,8,noArray(),2147483647,offset);
				itint++;
			}
		}
	}
...
// 		cvDrawContours( mask, m_externalContour.GetContourPoints(), CV_RGB(255,255,255), CV_RGB(255,255,255),0, cv::FILLED, 8,
// 						offset );
// 		vector<t_PointList> conts;
// 		conts.push_back(m_externalContour.GetContourPoints());
        drawContours(mask_mat,m_externalContour.GetContours(),-1,CV_RGB(255,255,255),cv::FILLED,8,noArray(),2147483647,offset);
		// draw internal contours
		t_CBlobContourList::iterator it = m_internalContours.begin();
		while(it != m_internalContours.end() )
		{
// 			cvDrawContours( mask, (*it).GetContourPoints(), CV_RGB(0,0,0), CV_RGB(0,0,0),0, cv::FILLED, 8,
// 						offset );
            drawContours(mask_mat,(*it)->GetContours(),-1,CV_RGB(0,0,0),cv::FILLED,8,noArray(),2147483647,offset);
			it++;
		}
	}
    ...
    cv::meanStdDev(image, mean, std, mask_mat );
...
void CBlob::FillBlob( Mat image, cv::Scalar color, int offsetX, int offsetY, bool intContours, const Mat srcImage){
	if(srcImage.data && intContours)
        assert(image.size()==srcImage.size() && image.type() == srcImage.type());
  {
    Rect bbox = GetBoundingBox();
    ...
    if(isJoined){
      ...
      // 				for(it;it!=en;it++){
      // 					drawContours(mask,(*it)->GetContours(),-1,255,cv::FILLED,8,noArray(),2147483647,offset);
      // 				}
      // 			}
      // 			srcImage(bbox).copyTo(temp,mask);
      // 			for(itBlob = joinedBlobs.begin();itBlob!=enBlob;itBlob++){
      // 				drawContours(image,(*itBlob)->m_externalContour.GetContours(),-1,color,cv::FILLED,8,noArray(),2147483647,drawOffset);
      // 			}
      ...
      if(intContours){
        ...
        drawContours(image,m_externalContour.GetContours(),-1,color,cv::FILLED,8,noArray(),2147483647,drawOffset);
        if(srcImage.data){
          ...
          for(it;it!=en;it++){
            drawContours(image,(*it)->GetContours(),-1,CV_RGB(0,0,0),cv::FILLED,8,noArray(),2147483647,drawOffset);
          }
        }
